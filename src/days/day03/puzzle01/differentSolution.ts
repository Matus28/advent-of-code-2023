...
const boundary = (c: Coord, width: number) => ({
  min: { x: c.x - 1, y: c.y - 1 },
  max: { x: c.x + width, y: c.y + 1 },
});
const within = (c: Coord, bounds: { min: Coord; max: Coord }) =>
  c.x >= bounds.min.x &&
  c.y >= bounds.min.y &&
  c.x <= bounds.max.x &&
  c.y <= bounds.max.y;
...
const isDigit = (ch: string) =>
  inRange(ch.charCodeAt(0) - "0".charCodeAt(0), 0, 9);
...
const lines = readFileSync(filePath).toString().trim().split("\n");
part1(lines);
part2(lines);
